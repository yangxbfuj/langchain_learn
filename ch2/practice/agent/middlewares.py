...
from ch2.practice.agent.message_sanitize import stringify_dialog, stringify_message
...
def _rate_limit_guard(state: AgentState) -> Optional[dict]:
    ...
    print(f"[中间件 - before_agent] rate_guard 触发检查：now={now}, last={last}")
    if now - last < 1.0:
        msg = AIMessage(content="请求太快啦，请 1 秒后再试。")
        print("[中间件 - before_agent] rate_guard 生效：拒绝请求并终止流程。")
        return {"messages": state.get("messages", []) + [msg], "jump_to": "end"}
    state["_last_ts"] = now
    print("[中间件 - before_agent] rate_guard 放行：更新最近访问时间。")
    return None
...
def _clarify_intent_guard(state: AgentState) -> Optional[dict]:
    ...
    if not isinstance(last, HumanMessage):
        print("[中间件 - before_model] clarify_missing_params 略过：最后一条消息不是用户。")
        return None

    text = (last.content or "").strip()
    print(f"[中间件 - before_model] clarify_missing_params 检查文本：{text}")
    ...
    if need_weather and not has_city:
        print("[中间件 - before_model] clarify_missing_params 提醒：天气查询缺少城市。")
    ...
    if need_math and not has_expr:
        print("[中间件 - before_model] clarify_missing_params 提醒：数学计算缺少表达式。")
...
def _post_model_annotate(state: AgentState) -> None:
    ...
    print("[中间件 - after_model] post_format_checker 已执行：标记模型输出已检查。")
...
def _log_after_agent(state: AgentState) -> None:
    ...
    print(f"[中间件 - after_agent] post_agent_logger 已执行：记录完成时间 {state['_last_done']}")
...
def _night_briefing_rule(request: ModelRequest) -> str:
    ...
    print(f"[中间件 - dynamic_prompt] night_briefing 检查当前小时：{hour}")
    base = request.system_prompt or ""
    if hour >= 23 or hour < 7:
        print("[中间件 - dynamic_prompt] night_briefing 生效：进入夜间简洁模式。")
...
def _timed_retry_wrapper_sync(
    ...
    print("[中间件 - wrap_model_call] timing_and_retry 同步封装开始。")
    t0 = time.time()
    try:
        resp = call(req)
    except Exception:
        print("[中间件 - wrap_model_call] timing_and_retry 捕获异常，尝试重试。")
        if retries > 0:
            resp = call(req)
        else:
            raise
    ...
    print("[中间件 - wrap_model_call] timing_and_retry 同步封装结束。")
    return resp
...
async def _timed_retry_wrapper_async(
    ...
    print("[中间件 - awrap_model_call] timing_and_retry 异步封装开始。")
    t0 = time.time()
    try:
        resp = await call(req)
    except Exception:
        print("[中间件 - awrap_model_call] timing_and_retry 捕获异步异常，尝试重试。")
        if retries > 0:
            resp = await call(req)
        else:
            raise
    latency_ms = round((time.time() - t0) * 1000, 2)
    _record_latency(req, latency_ms)
    print("[中间件 - awrap_model_call] timing_and_retry 异步封装结束。")
    return resp
...
class ConversationGuardMiddleware(AgentMiddleware):
    ...
    def before_agent(self, state: AgentState, runtime=None, **kwargs) -> dict | None:  # noqa: ARG002
        print("[中间件 - before_agent] ConversationGuard.before_agent 检查输入长度。")
        ...
        if isinstance(last, HumanMessage) and len(last.content or "") > self.max_input_chars:
            ...
            print("[中间件 - before_agent] ConversationGuard.before_agent 拒绝：输入过长。")
    ...
    def before_model(self, state: AgentState, runtime=None, **kwargs) -> dict | None:  # noqa: ARG002
        print("[中间件 - before_model] ConversationGuard.before_model 检查消息数量。")
        if len(state.get("messages", [])) > 20:
            print("[中间件 - before_model] ConversationGuard.before_model 生效：历史消息过多，提示先总结。")
            return {"system_prompt": "历史较多，请先简要总结要点，再给出答案。"}
        return None

    def after_model(self, state: AgentState, runtime=None, **kwargs) -> None:  # noqa: ARG002
        print("[中间件 - after_model] ConversationGuard.after_model 检查输出长度。")
        ...
        if isinstance(last, AIMessage) and len(last.content or "") > self.max_output_chars:
            ...
            print("[中间件 - after_model] ConversationGuard.after_model 生效：输出过长已截断。")
